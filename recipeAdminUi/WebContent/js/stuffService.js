...
angular.module('starter.services', [])
    ...
    .service('MainService', function ($http, $q) {
        this.getSampleList = function (sampleReq) {
            var dfd = $q.defer();
            var url = URL_BASE + "/admin/stuff/list";
...
//            	withCredentials:true		
	...
	.service('getModalView', function (id) {
		alert('getModalView' + id );
		console.log("getModalView" + id);
	    
	   /* var dfd = $q.defer();
	    var url = URL_BASE + "/admin/stuff" + id;
	   $http.post(url, sampleReq).success(function (result) {
	            if (result.code == 0) {
	                dfd.resolve(result.data);
	            }else {
	               dfd.reject(result.message);
