...
//		if(req.isIsCreate()) {
//			list = rcmdMapper.getRecommendStuffsforTasteSubCreate(req);	
//		} else {
...
//		}
	...
	public ResultWithData getAiRecipeDetail(AiRecipeDetailReq req) {
		...
		if(basicStuffList != null && !basicStuffList.isEmpty()) {
			...
			// 2개 식자재로 찾는다.
			List<RcmdStuffPMIVO> addStuffList = rcmdMapper.getStuffsPMI2Var(req);
			int stnum = 0;
...
//			this.setNewRecommendStuffs((RcmdRecipeReq) req);
	...
	public RcmdRecipeVO getCreatedRecipe(AiRecipeDetailReq req) {
		
		req.seperateIdsforCreate();
		List<RcmdStuffPMIVO> basicStuffList = this.getRecommendStuffsforTaste(req);
		
		List<RcmdRecipeVO> baseRecipeIds = null;
		int num = 0;
		int random = 1;
		...
		if(basicStuffList != null && !basicStuffList.isEmpty()) {
			...
			if(num >=5) num = 5;
			random = (int) (Math.random() * num);
			AiRecipeDetailReq req2 = new AiRecipeDetailReq();
			req2.addStuffId(basicStuffList.get(random).getStuffId1());
			req2.addStuffId(basicStuffList.get(random).getStuffId2());
			req2.seperateIds();
			
			baseRecipeIds = recipeSetMapper.getBaseRecipes(req2);
		} 
		
		if(baseRecipeIds == null || baseRecipeIds.isEmpty()) {
			baseRecipeIds = recipeSetMapper.getBaseRecipes2(req);
		}
		
			
		if(baseRecipeIds == null || baseRecipeIds.isEmpty()) return null;
		
		if(baseRecipeIds != null && !baseRecipeIds.isEmpty()) {
			RcmdRecipeVO baseRecipeVO = null;
			String baseRecipeId = null;
			...
			if(AIRecipeSurpriseLevel.LOW.equals(req.getAiSurpriseLevel())) {
				if(baseRecipeIds != null) {
					...
					while(true) {
						random = (int) (Math.random() * num);
						baseRecipeVO = baseRecipeIds.get(random);
						baseRecipeId = baseRecipeVO.getRecipeId();				
			...
			String recipeName = null;
			String imageUrl = null;		
			RecipeMasterVO recipevo = recipeMapper.getRecipeDetailMaster(new RecipeReq(baseRecipeId, req.getUserId()));
			if(recipevo != null) {
				recipeName = recipevo.getRecipeName();
				imageUrl = recipevo.getImageUrl();
			}	
			...
			// 식자재가 최소 2개 이상 지정되었다면 첫번쨰, 두번째 식자재를 각각 지정한다.
			// 식자재가 1개 인 경우 다른 하나를 찾는다.
			
			String stuffId1 = "";
			String stuffId2 = "";
			String stuffName1 = "";
			String stuffName2 =  "";
			List<RcmdStuffPMIVO> addStuffList = null;
			if (req.getStuffCount() == 1) {
				addStuffList = rcmdMapper.getStuffsPMI1Var(req);
			} else {
				addStuffList = rcmdMapper.getStuffsPMI2Var(req);	
				
				// 만약 없다면, 하나로 그대로 진행한다.
				if(addStuffList == null || addStuffList.isEmpty()) {
					addStuffList = rcmdMapper.getStuffsPMI1Var2(req);
				}
			}
			
			if(addStuffList == null || addStuffList.isEmpty()) return null;
			
			int stnum = 0;
			stnum = addStuffList.size();
			if(stnum >=3) stnum = 3;
			int strandom = (int) (Math.random() * stnum);
			stuffId1 = addStuffList.get(strandom).getStuffId1();
			stuffId2 = addStuffList.get(strandom).getStuffId2();
			stuffName1 =  addStuffList.get(strandom).getStuffName1();
			stuffName2 =  addStuffList.get(strandom).getStuffName2();
			...
			// 추가할 후보를 찾는다.
			if(addStuffList != null && !addStuffList.isEmpty()) {
				AiRecipeDetailReq findRecipeReq = new AiRecipeDetailReq();
				
				findRecipeReq.setUserId(req.getUserId());
				findRecipeReq.setBaseRecipeId(baseRecipeId);
				findRecipeReq.addStuffId(stuffId1);
				findRecipeReq.addStuffId(stuffId2);
				findRecipeReq.seperateIds();	
				findRecipeReq.setPageSize(100);
				
				List<RcmdRecipeVO> list = recipeSetMapper.getAddRecipes(findRecipeReq);
...
//			this.setNewRecommendStuffs((RcmdRecipeReq) req);
			...
			baseRecipeVO.setRecipeId(req.getRecipeId());
			baseRecipeVO.setRecipeName(recipeName);
			baseRecipeVO.setCategoryName(recipevo.getCategoryName());
			baseRecipeVO.setCookDifficulty(recipevo.getCookDifficulty());
			baseRecipeVO.setCookingTime(recipevo.getCookingTime());
			baseRecipeVO.setImageOriginalUrl(recipevo.getImageOriginalUrl());
			baseRecipeVO.setImageThumbUrl(recipevo.getImageThumbUrl());
			baseRecipeVO.setImageUrl(recipevo.getImageUrl());
			baseRecipeVO.setHealthLevel(recipevo.getHealthLevel());
			baseRecipeVO.setIsAIRecipe('Y');
			baseRecipeVO.setIsPopular(recipevo.getIsPopular());
			baseRecipeVO.setIsTaste(recipevo.getIsTaste());
			baseRecipeVO.setIsRcmdbyHeath(recipevo.getIsRcmdbyHeath());
			baseRecipeVO.setRateHealth(recipevo.getRateHealth());
			baseRecipeVO.setRatePopular(recipevo.getRatePopular());
			baseRecipeVO.setRateTaste(recipevo.getRateTaste());
	...
	public RcmdRecipeVO getRecommendRecipebyPopular(RecipeSetReq req) {
		return rcmdMapper.getRecommendRecipebyPopular(req);
	}

	public RcmdRecipeVO getRecommendRecipebyTaste(RecipeSetReq req) {
		return rcmdMapper.getRecommendRecipebyTaste(req);
	}

	public RcmdRecipeVO getRecommendRecipebyHealth(RecipeSetReq req) {
		return rcmdMapper.getRecommendRecipebyHealth(req);
	}
