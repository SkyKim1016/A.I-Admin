...
public class RecipeRcmdService {
	...
	protected SearchFilterReq setSearchFilterReq(SearchFilterReq rSearchFilterReq) {
		...
		List<SearchFilterUnitReq> list = new ArrayList<SearchFilterUnitReq>();
				
		for(SearchFilterUnitReq unit : rSearchFilterReq.getConditions()) {
			if(unit.getFilterType().equals(SearchFilterType.POPULAR)) {
				SearchFilterUnitReq realUnit = new SearchFilterUnitReq();
				realUnit.setKeyCol("count_view");
				realUnit.setAscDescType(AscDescType.DESC);
				list.add(realUnit);			
				
			} else if(unit.getFilterType().equals(SearchFilterType.TASTE)) {
				SearchFilterUnitReq realUnit = new SearchFilterUnitReq();
				realUnit.setKeyCol("sim.sim");
				realUnit.setAscDescType(AscDescType.DESC);
				list.add(realUnit);

				realUnit = new SearchFilterUnitReq();
				...
				realUnit.setKeyCol("sim.sim");
				realUnit.setAscDescType(AscDescType.DESC);
				realUnit.setFilterType(unit.getFilterType());
				list.add(realUnit);
	...
	protected SearchSortReq setSearchSortReq(SearchSortReq rSearchSortReq) {
		...
		for(SearchSortUnitReq unit : rSearchSortReq.getConditions()) {
			if(unit.getSortType().equals(SearchSortType.POPULAR)) {
				...
				realUnit.setAscDescType(AscDescType.ASC);
				list.add(realUnit);				
			} else if(unit.getSortType().equals(SearchSortType.CREATED)) {
				
				SearchSortUnitReq realUnit = new SearchSortUnitReq();
				realUnit = new SearchSortUnitReq();
				realUnit.setKeyCol("sim.sim");
				realUnit.setAscDescType(AscDescType.DESC);
				realUnit.setSortType(unit.getSortType());
				list.add(realUnit);
	...
	public ResultWithData getRcmdRecipeforMina(RcmdRecipeReq req) {
		...
		unit.setKeyCol("sim.sim");
		unit.setAscDescType(AscDescType.DESC);
		conditions.add(unit);

		unit = new SearchFilterUnitReq();
		unit.setKeyCol("sim.id_recipe");
		unit.setAscDescType(AscDescType.ASC);
		conditions.add(unit);
