...
public class RcmdRecipeReq extends PageReq {
	...
	public String getQueryString() {
		...
		if(this.sortReq != null && this.sortReq.getConditions() != null && !this.sortReq.getConditions().isEmpty()) {
			for(SearchSortUnitReq unit : this.sortReq.getConditions()) {
				if(SearchSortType.CREATED.equals(unit.getSortType())) {
		...
		if (this.stuffIds != null && !this.stuffIds.isEmpty()) {
			...
			int i = 0;
			for(String id : this.stuffIds) {
				strJoin += " join (select id_recipe from mst_recipe_stuff_mt where id_stuff = '" + id + "') a";
